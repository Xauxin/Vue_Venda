            ...
            <v-col cols="12">
                <v-card>
                    <v-row align="center" class="px-4 py-2">
                        <v-col cols="11">
                            <p class="text-h4">{{ bordados[BordadoAscessado].local.replace("_", "") }}</p>
                        </v-col>
                                        ...
                                        <v-col cols="2" v-for="bordado in bordadosList" v-bind:key="bordado.codigo">
                                            <v-tooltip :text="bordado.Nome" location="top">
                                                <template v-slot:activator="{ props }">
                                                    <v-card @click.prevent="selcionaBordado(bordado)" v-bind="props"
                                                        height="100%" hover>
                                            ...
                                            <v-table density="compact">
                                                <tbody>
                                                    <tr v-for="(item, key) in BordadoPreSelecionado" :key="key">
                                                        <td v-if="!campos_a_nao_serem_mostrados.includes(key)">{{ key }}
                                                        </td>
                                                        <td v-if="!campos_a_nao_serem_mostrados.includes(key)">{{ item }}
                                                        </td>
                                                    </tr>
                                                </tbody>
                                            </v-table>
...
export default defineComponent({
    ...
    methods: {
        ...
        adicionaBordado(bordadoASerAdicionado: IBordado) {
            this.bordados[this.BordadoAscessado].bordado = bordadoASerAdicionado
            this.dialog = false
        }
    },
    setup() {
